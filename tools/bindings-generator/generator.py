...
class Generator(object):
    def __init__(self, opts):
        ...
        if self._hpp_headers is not None:
            for header in self._hpp_headers:
                reldir = os.path.join(header, self.outdir)
                self.hpp_headers.append(reldir)
        cocos_root = os.path.normpath(
            os.path.join(os.path.dirname(__file__), "../.."))
        for header in self.headers:
            reldir = os.path.relpath(header, self.outdir)
    ...
    for t in targets:
        ...
        for s in sections:
            ...
            gen_opts = {
                ...
                'outdir': config.get(s, 'outdir', raw=False, vars= dict(userconfig.items('DEFAULT'))) if config.has_option(s, 'outdir') else outdir,
